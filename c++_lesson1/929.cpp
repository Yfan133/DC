/*
			_CrtDumpMemoryLeaks();	// Windows提供的内存泄露检测api，可以在输出窗口看到内存泄露

	写博客有没有内存泄露，如何检测内存泄露在哪，为什么自定义类型(类)没有匹配使用会崩溃？

new/delete 是关键字，也可以成为操作符--new 操作符
	把new/delete重载之后可以，打印new/delete创建的位置，因此可以查找内存泄露
	
malloc开辟的类空间转换成对象：
	1.new(pt) Test(100) 用构造初始化
	2.释放时：
		pt->~Test();	// 先调用析构
		free(pt);		// 再调用释放空间

	小结：
		new 相当于---> 先malloc，再new+构造
...
浅拷贝：将一个对象中的内容原封不动的拷贝到另一个对象中，用了同一块物理内存，在释放时会产生6号信号 SIGABRT 而崩溃，double free
