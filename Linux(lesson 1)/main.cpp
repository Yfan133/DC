...
为了实现同步因此队列的长度是定长的
从网络来的数据，直接调用push插入线程池队列，然后在处理完成之后，再发送回，发送队列，之后发送线程，回到网络
通过了一个队列(也可以叫缓冲)，把前台和后端解耦开来
...
5.在设置标志位为true的函数中，调用boardcast通知PCB等待队列中被阻塞的线程
...
*/
/*
如果一直开辟空间不释放，可能导致申请内存失败被操作系统干掉
队列是可以无限扩容的，因此人为定义一个capacity。
写多线程代码
1.考虑业务核心逻辑
...
sem
初始化消费者队列时，为0代表当前没有资源可用，等生产者生产后调用post会使消费者计数器+1
*/
/*
