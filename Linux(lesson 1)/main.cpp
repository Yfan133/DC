...
/*
进程控制4个点：创建，终止(退出)，等待，替换
创建：pid_t pid=fork();---复制父进程的pcb创建一个子进程，代码共享数据独有
写时拷贝技术：起初子进程和父进程用了同一块物理内存(避免给子进程开辟空间而不用造成资源浪费)，在父子进程物理内存将发生变化时，操作系统给子进程重新开辟空间，但父子进程的虚拟地址会是相同的
终止：退出一个进程
退出进程方式：
1.

等待：父进程等待子进程退出
int wait(int *status);  阻塞接口，若没有子进程退出则会一直等待直到有子进程退出才会调用返回
阻塞：为了完成一个功能，发起一个调用，若不具备完成功能的条件，则调用一直等待
非阻塞：为了完成一个功能，发起一个调用，若不具备完成功能的条件，则调用立即报错并返回
返回值：成功则返回处理过的子进程的pid，失败则返回-1(比如没有子进程退出)
int waitpid(int pid,int *status;int option);与wait不同，默认为非阻塞
wait和waitpid的不同之处：
1.wait
