...
画个图
	生产者与消费者模型的优点：
		可以解耦合：生产者只负责生产，消费者只负责消费，生产者和消费者都是通过队列进行交互，即生产者和消费者各一个队列，互不干涉。
...
posix信号量
		...
		2)初始化：
			sem_init(sem_t* sem,int pshared,int value)
				...
				pshared：表示当前信号量是用在进程间还是线程间
					 0：线程间使用  
					!0：进程间使用
					ipcs中第三个信号量：本质：在共享内存中创建的一块，因此可以进程间通信
					当使用sem_init初始化信号量为进程间时，会在内核中创建一块共享内存，来保存信号量的数据结构，其中资源计数器，PCB等待队列都是在共享内存当中维护的。
					所以我们调用唤醒或者等待接口的时候，就通过操作共享内存实现了不同进程之间的通信，进而实现不同进程之间的同步与互斥。
					线程间：全局变量则都能访问到，因为同一块虚拟地址空间
				value：实际资源数量，用于初始化信号量当中资源计数器，表示实际的资源数量
		3)等待：
			sem_wait(sem_t* sem)-->阻塞等待
			sem_trywait(sem_t* sem)-->非阻塞等待
			sem_timedwait(sem_t* sem,const struct timespec* timeval)-->带有超时时间的等待
			总结：如果调用等待接口进行获取信号量，不论是否获取成功，都会对资源计数器-1
				  如果：！！！！
		4)唤醒：
			int sem_post(sem_t* sem);发布信号量，表示资源使用完成了，需要归还资源或者生产者重新生产了一个资源，对信号量中的资源计数器+1，唤醒PCB等待队列当中的PCB。
			每发布一个信号：资源计数器进行+1，wait接口里面-1
		5)销毁：
			sem_des(sem_t* sem)；释放信号量开辟的内存
	4.如何保证同步和互斥，画个图：信号量实现互斥和同步的原理图
		同步：
			1)初始化的时候根据资源的数量来进行初始化posix信号量当中的资源计数器
		互斥：
			2)初始化时，必须初始化

		互斥：线程A先获取信号量，然后信号量的资源计数器-1，多个获取资源计数器会一直进行-1，(计数器小于0)暂时都被阻塞，绝对值就是当前等待获取的线程数。
		同步：若有多个资源计数器(例如为2)，则可实现多个线程同时访问临界资源。
			  线程B访问完成之后，则释放资源，1.调用sem_post(sem_t*)资源计数器+1，2.通知通知PCB等待队列，线程C访问临界资源
	
	用posix实现生产消费模型
		1.环形队列-》数组实现
			插入位置：(write+1)%capatity
		2.安全队列
			同步：
				写：sem_init(&SemWrite,0,数组元素个数)初始化容量
				读：sem_init(&)初始化0
			互斥：
	...
	4.使用信号量
		...
		使用信号量实现生产者与消费者模型
			线程安全的队列(数组+先进先出的特性)
			画个图：posWrite、posRead
			用PosWrite=(POSWrite+1)%capacity 计算位置，循环读写
		sem_t lock；互斥-->初始化后资源数为1
		sem_t Consume;消费者PCB等待队列 同步
			read：sem_init(&Consume，0，0)；最后一个参数要初始化为0，表示当前没有资源可用
			      sem_post(&Product)：空闲的空间+1,通知生产者PCB等待队列
