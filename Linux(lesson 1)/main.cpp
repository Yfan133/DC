...
读写锁：和互斥锁很相似，但是比互斥锁高效
	...
	3.加锁规则
		...
		并行：1.多个线程可以同时占有读模式的读写锁，在读写锁的内部有一个引用计数
...
线程池：
	...
	如何让相同入口函数的线程，处理不同的请求？
		1.switch case：处理大量不同需求时，比较麻烦
		2.向线程池抛入数据的时候，将处理该数据的函数一起抛入(函数地址)，线程池当中的线程，只需要调用传入的函数处理数据即可
