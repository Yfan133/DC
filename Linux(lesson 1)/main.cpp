...
1.管道
	...
	管道本质就是在内核当中开辟的一块内存，也可以称之为内核缓冲区，这段空间没有标识符(pid)
	用户态        ps aux | grep xxx
...
管道在内存的缓冲区有多大（能存多少字节）：64k  pipe_buf==4k 同时写入时，最大写入4k就要切换另一个，写入4k，再切换回来
读写端默认为阻塞(写入的数据，读两次，第二次读会阻塞)，非阻塞O_NONBLOCK
int flag = fcntl(fd,F_GETFL);
fcntl(fd,F_SETFL,flag | NONBLOCK); 为什么要与，NONBLOCK八进制是400，转换成二进制有1的位置是代表非阻塞，和flag与就能非阻塞还带有flag的属性
...
返回值：返回共享内存的操作句柄
问题：标识符：用来表示共享内存，它是一个地址吗？
不是，在Linux中可以把它理解成一个内存段名

共享内存段名和共享内存的操作句柄的区别：
操作句柄(返回值ret)：通过句柄，进程可以对共享内存进行操作(附加，分离，删除等)
...
3.将共享内存附加到一个进程：
	...
	shmid:共享内存操作句柄
	shmaddr：指定连接的地址，写NULL时操作系统自动选择一个地址，并把进程附加到共享内存中
	shmflg：一般都为0
	SHM_RDONLY：只读
	0：可读可写
返回值：成功返回指针，指向共享内存第一个节，失败返回-1
...
5.操作共享内存
	...
	cmd:将要采取的动作（有三个可取值），
	IPC_STAT：读取当前共享内存的属性信息(比如共享内存的大小，创建时间等等)，放在buf里
	IPC_SET：设置共享内存的属性信息，创建一个结构体shmid_ds buf，用他的内容去设置
	IPC_RMID：删除共享内存，buf可为NULL
	buf:字符数组
