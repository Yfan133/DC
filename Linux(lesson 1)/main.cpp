...
SIGCHLD：
	...
	注意：不能让17号信号SIGCHLD之外的信号定义为调用该函数，若其它信号的handler也指向这个自定义函数，收到该信号时就会阻塞内核执行流(没有子进程退出)
volatile:使内存保持可见性，从内存中读取值
	-o2优化后一般为了提高执行效率，CPU是在寄存器中取值，但其实该值在内存中内已经改变了，寄存器里的没有改变
	使用volatile可以一直都取出的是内存中的值
*/
/*
多线程
	程序中不只有一个main函数执行流，有很多个执行流在同一时刻拿着不同的CPU进行运算，并行，这样可以
