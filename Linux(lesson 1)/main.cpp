...
线程控制：创建，终止，等待，分离
	...
	画个图
	总结：pthread_t是线程ID，在内核的本质就是线程独有空间的首地址！
...
如何保证线程的安全性：
	互斥：在同一时刻只能有一个执行流访问临界资源(关闭厕门，拒绝其他人继续进去)
	同步：程序对临界资源的合理访问,也就是顺序访问，第一个访问完成接着第二个去访问(排队上厕所)
互斥锁：是一个阻塞接口
