...
/*
sigqueue队列为什么不是先进先出？
	do_signal函数处理信号时，会先通过pending和block位图计算该处理哪一个节点了，然后再去sigqueue队列中取节点
线程入口函数可以传递什么参数？
1.临时变量，为什么会出现，2,3甚至10的情况。不能传
	临时变量的作用域为for循环，出了循环之后，就在栈帧上被释放掉了。而此时传进线程的地址就是一块非法的地址，如果在栈上在这块空间新开辟了，就会报错。(这和之前C++中哪一块一致想想！)
2.结构体对象，不能传
3.结构体指针，可以
	堆上开辟的空间必须由我们手动释放，因此传堆上开辟的空间是没有问题的，只需要记住在线程退出的位置释放，防止内存泄露
4.this指针，可以
总结：只要是堆上开辟的空间(传指针)都可以，理解函数传参时强转，其实是拷贝(重新创建)了一份临时的变量，例如这里创建了一个临时的void*指针变量。
pthread_cancel()并不是立即退出！！！！要等待一小会
top -H -pid
...
volatile无法解决这里的问题，这里是从内存中取出数据之后，线程运行时间停止。
而volatile是拿数据的时候从内存中取。
...
线程控制：创建，终止，等待，分离
	...
	画个图
	总结：pthread_t是线程标识符，在内核的本质就是线程独有空间的首地址！
...
线程终止的方式：
	...
	3.pthread_cancel(pthread_t thread)：结束指定的线程，结束传入的线程标识的那个线程，也就是线程标识符为thread的线程，这个标识符可以是pthread_create()函数的出参，也可以用pthread_t pthread_self()获取自己的
		thread:线程的标识符
...
线程安全：(多个线程同时运行，访问临界资源，不会导致程序的结果产生二义性则称线程安全)临界资源：多线程共享的资源，也就是可能同时访问到的资源，若多个线程同时访问可能造成二义性
	线程对临界资源的访问：在临界区当中对临界资源是非原子性操作，也就是可能在中途会被打断，并保存上下文数据和程序计数器
	原子性操作：操作是一步完成的，意思就是：执行中没有被打断并保存程序计数器和上下文信息，等下一次执行
	...
	互斥锁在内核中是什么？至少是一个所有线程共享的内容。

如何保证线程的安全性：
	互斥(互斥锁加锁)：在同一时刻只能有一个执行流访问临界资源(关闭厕门，拒绝其他人继续进去)
	同步(互斥锁解锁)：程序对临界资源的合理访问,也就是顺序访问，第一个访问完成接着第二个去访问，这里的第二个是操作系统调度的，抢占式执行(厕门打开，抢上厕所)
互斥锁：是一个阻塞接口
	...
	那么引出一个问题：互斥量计数器本身也是一个变量，那这个变量的取值进行++、--时，会是原子性的操作吗？
		...
		画个图：互斥变量的内容变化
		寄存器中的值和内存中的值交换，这个操作是原子性的，在汇编中的指令是xchgb

总结：完整的解释加锁流程
	  1.当计数器中的值为1时，表示可以加锁，寄存器和内存中的0交换，相当于给计数器--了
	  2.当计数器中的值为0时。表示不能加锁，寄存器和内存中的0交换，没有对计数器的值做到修改，因此请求加锁的执行流阻塞等待，直到计数器为1
问题：计数器和寄存器之间怎么直接挂钩的？？？为什么计数器中的值是多少寄存器中的值也是多少？
...
互斥锁的使用流程：
	...
	3.加锁
		...
		3.int pthread_mutex_timedlock(pthread_mutex_t* mutex,const struct timespec* abs_timeout);阻塞等待一定时间之后，若还没获取互斥锁，则报错返回ETIMEOUT
			mutex:传阻塞变量的地址，&lock
			abs_timeout:加锁超时时间，超过时间报错返回，有两个变量：一个是秒，一个是纳秒
	4.解锁
...
在哪加锁？在哪解锁？不解锁会造成什么问题？

注意：在访问临界资源之前进行加锁操作，
用pstack -进程号 可以看到main函数卡在pthread_join一直等待，且有一个线程退出了，
原因：解锁位置错误，最后一张票拿出时，一个拿到票且拿到互斥锁的线程break退出，其它的线程一直被卡住，拿不到互斥锁，英雌阻塞
注意:在所有退出的地方都应该进行解锁操作
为什么拿到票100就卡住了，原因：自己拿到之后去循环，自己卡住了自己，死锁！！
总结：加锁操作有点像，我们去某个地方(厕所)设置一个门卫，门里有人则不放行
*/
