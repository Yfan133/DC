	...
	条件变量接口：
		...
		4.唤醒接口：唤醒PCB等待队列当中的执行流进行出队
			int pthread_cond_signal(pthread_cond_t* cond)
				唤醒一个PCB等待队列当中的线程：1.如果队列中有多个线程则根据线程优先级确定唤醒顺序。2.如果优先级一致，则依据等待时长确定出队顺序				
...
这里有几个问题关于：pthraed_cond_wait(&cond,&lock);
	1.为什么需要传入互斥锁？(互斥)
		简要说：同步只保证了各个执行流对临界资源访问的合理性，而并没有保证互斥属性，因此需要和互斥锁进行配合，完成各个执行流对临界资源访问的同步和互斥属性。
		具体说：先把调用接口的线程入PCB等待队列，再解锁互斥锁。让生产线程去访问，然后在收到信号唤醒时，出队，继续加锁并访问临界资源。这个函数内部封装了解锁和加锁的操作
...
画个图
	生产者与消费者模型的优点：
		可以解耦合：生产者和消费者都是通过队列进行交互，即生产者和消费者各一个队列，互不干涉。
		支持忙闲不均：队列起到了缓冲作用(男生追女生带苹果)
		支持并发：消费者只关心队列中是否有数据可以进行消费
				  生产者只关心队列中是否有空闲的节点进行生产
...
//2.循环判断g_val，程序运行一截子之后被卡住。根本原因：生产者和消费者用了同一个等待队列，而singal接口按时间顺序唤醒PCB，也就是可能消费者PCB唤醒了消费者PCB，造成卡死。
