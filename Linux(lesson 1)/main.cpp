...
线程的缺点：画一个滑稽吃鸡
	...
	5.性能损失，一个进程拥有很多个线程时，操作系统会不停的进行切换调度，而每次切换都要保存当前代码执行信息和程序计数器(程序执行到代码哪一行)、上下文数据等。(重新拿到时间片)回调时占用CPU执行处理上下文信息，程序计数器
	总结：
...
线程控制：创建，终止，等待，分离
	...
	创建：
		...
		用例：pthread_creat(&tid,NULL,func,NULL);
			thread:线程标识符pthread_t，是一个出参，和线程id并不是一回事，pthread_t是线程在共享区独有空间的首地址，通过这个标识符可以对当前线程进行操作，调用pthread_creat作为出参返返回给调用者。这个标识符也就是我们用pthread_t tid;创建出来，并传入create函数的"出参"
...
线程终止的方式：
	...
	注意：线程在默认创建的时候，默认属性当中认为线程是joinable的
		joinable属性:当线程在退出的时候，需要其他线程来回收该线程的退出信息(资源)，如果没有任何线程去回收，则共享区中对于退出线程的空间还是保留的，退出资源没有完全释放造成内存泄露。(类似僵尸进程)
...
线程安全：
	多个线程同时运行，访问临界资源(同一时刻，该资源只能被一个线程(执行流)所访问)，不会导致程序的结果
*/
//1.传临时变量的代码
//2.传结构体指针的代码
