...
线程控制：创建，终止，等待，分离
	...
	2.getpid()获取的是进程ID即tgid，不管是在工作线程还是在主线程。tid也就是LWP，pthread_self()获取的就是pthread_t，即首地址
	3.pthread_creat是C库中函数，在用户态进行运行。创建出来的是用户级线程，在内核中通过LWP进行调度
	4.为了避免越界访问，只能给arg传堆上开辟的空间。
	5.轻量级进程就是一个PCB，这个PCB的
...
总结：完整的解释加锁流程
	  1.当计数器中的值为1时，表示可以加锁，寄存器中的0和内存中计数器的1交换，相当于给计数器--了
	  2.当计数器中的值为0时。表示不能加锁，寄存器中的0和内存中计数器的0交换，没有对计数器的值做到修改，因此请求加锁的执行流阻塞等待，直到计数器为1
	...
	条件变量接口：
		...
		3.等待接口：将调用该等待接口的执行流放到PCB等待队列当中去，进行等待
			int pthread_cond_wait(pthread_cond_t* cond,pthread_mutex_t* mutex)
				...
				问题：为什么需要传入互斥锁
				因为：需要在wait接口里面，解锁互斥锁，内部封装了解锁和加锁的操作
...
*/
/*
在做完面之后用pthread_cond_signal(&cond)唤醒顾客	
