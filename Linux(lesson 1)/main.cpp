...
volatile无法解决这里的问题，这里是从内存中取出数据之后，线程运行时间停止。而volatile是拿数据的时候从内存中取。

pthread_mutex_t ：结构体类型
PTHREAD_MUTEX_INITIALIZER ：宏定义了一个结构体的值
非阻塞接口：trylock，加锁失败则返回EBUSY，一般要循环加锁，返回0加锁成功则退出循环，防止因加锁失败而继续往下执行
在访问临界资源之前，就要加锁，防止tickets减成-1
同步：本质就是保证了顺序去访问临界资源(一个线程去执行一次就切换)，保证了各个线程对临界资源访问的合理性
条件等待：
	本质：PCB(task_struct结构体)等待队列+一堆接口(等待接口、唤醒接口)
用continue实现同步，疯狂判断，浪费CPU资源
入等待队列之后，挂起等待
发送信号是在解锁之后！！！

如果有多个做面人和顾客：
	会有错误，第二个厨子出来直接执行g_val++了
如果循环判断
	但会卡死，为什么会卡死？
...
线程安全：(多个线程同时运行，访问临界资源，不会导致程序的结果产生二义性则称线程安全)临界资源：同一时刻只能有一个执行流访问，若多个线程同时访问可能造成二义性
	线程对临界资源的访问方式：在临界区当中对临界资源的访问是非原子性操作，也就是可能在中途会被打断，并保存当前上下文数据和程序计数器，等待下次处理
...
如何保证线程的安全性：
	...
	同步：程序对临界资源的合理访问,也就是顺序访问，一个执行流访问一次就切换。第一个访问完成接着第二个去访问，这里的第二个是操作系统调度的，抢占式执行(厕门打开，抢上厕所)。
互斥锁：是一个阻塞接口
	...
	那么引出一个问题：互斥量计数器本身也是一个变量，那这个变量的取值进行++、--时，会是原子性的操作吗？
		...
		画个图：互斥变量的内容变化
		寄存器中的值(默认给0)和内存中互斥量计数器的值进行交换，这个操作是原子性的，在汇编中的指令是xchgb，因此互斥量计数器中的操作是原子性的
...
互斥锁的使用流程：
	1.定义互斥锁
		pthread_mutex_t:互斥锁变量类型，mutex(互斥锁)
	...
	死锁：
		...
		2.死锁的4个必要条件(形成死锁的4种情况)：
			互斥条件：每一把锁只被一个执行流所占用(4个线程最后剩3个，且程序卡住)
			请求与保持条件：当期执行流已经占用了一个互斥锁，还申请新的互斥锁(环型死锁)
			循环等待条件：若干执行流在请求所资源的情况下，形成了一个闭环(输出100后卡住)
			不可剥夺：只有当前拿着互斥锁的线程可以释放该资源
		3.避免死锁的方法
			1.破坏必要条件：(例如循环锁时，用trylock)
			2.加锁顺序一致：线程A和B，A先加锁解锁1，然后B再加锁解锁1。
			3.记得解锁：线程可能退出的地方都要进行解锁
			4.一次性分配资源：执行流在完成某件事时，一次性把需要的锁资源都拿过来(拿过来之前已经被解锁)，用完再解锁

同步：同步保证了各个执行流对临界资源访问的合理性，通俗来讲：也就是一个执行流访问一次，就切换另一个访问(这里另一个是抢占式执行)。
条件变量：什么是条件变量？
	...
	条件变量接口：
		...
		4.唤醒接口：通知PCB等待队列当中的执行流来进行访问临界资源
			int pthread_cond_signal(pthread_cond_t* cond)-->唤醒至少一个PCB等待队列当中的线程(如果唤醒多个则抢占式执行)
				
			int pthread_cond_broadcast(pthread_cond_t* cond)唤醒所有PCB等待队列当中的线程(抢占式执行)
...
完整的解释做面程序：
	顾客(线程A)进行加锁后两种情况：
		1.noodles==0，表示需要不满足情况

问题：为什么需要传入互斥锁
	因为：需要在wait接口里面，解锁互斥锁，然后在收到信号唤醒时，继续加锁然后访问临界资源。这个函数内部封装了解锁和加锁的操作
问题：是先放入PCB等待队列还是先解锁？
	是先放入PCB等待队列，因为一解锁其它线程马上拿到锁资源，然后给PCB等待队列发送信息，但这时PCB队列是空的，刚解锁的还没有入PCB队列
问题：该接口的内部实现逻辑？
...
问题：唤醒之后，应该怎么做
	1.从PCB队列中移除出来
	2.抢占互斥锁
		拿到互斥锁，pthread_cond_wait函数返回
		没拿到，阻塞在pthread_cond_wait函数内部抢锁的逻辑当中
			当卡在pthread_cond_wait内部抢锁逻辑的执行流一旦时间片耗尽，程序计数器中就保存抢锁指令，上下文数据当中保存的就是寄存器中的0(还没抢到锁)。
			当再次拿到时间片，从程序计数器和上下文信息当中恢复抢锁的逻辑
问题：
...
*/
/*
注意：顾客这边一解锁，厨子那边就自动加锁了，因为厨子是卡在while(1)下面的加锁。
...
//lock
//环锁,执行流A和执行流B都卡在加锁的位置。
//吃面和做面保证了互斥，但没有保证同步(也就是可能吃了很多面，吃到了负数去)
//吃面和做面一个个顺序执行
//说到底同步就是保证了执行流的顺序访问，一人一次
int main()
{
	return 0;
}