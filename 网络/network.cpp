		...
		3.监听连接：
			int listen(int sockfd，int backlog)；
				...
				backlog：已完成连接队列的大小，也就是完成三次握手之后的连接会被放进去
					...
					2.已完成连接队列：处于我收完毕(连接建立完毕)同时没有被获取的连接
					backlog指定已经完成连接的队列大小，如果已完成连接队列被打满，处理方式就是丢弃掉新来的连接
					使用Recv，从已完成连接队列当中获取创建完成的新连接
//////////////////////////////////////////////////////////////////////////////////////////////////
		4.接收连接
			int accept(int sockfd)
				如果以完成链接队列当中没有完成三次握手的链接，则连接阻塞!!!!
				等握手成功则，进入Recv等待cli发送数据
		5.
			...
			ssize_t recv(int sockfd,void* buf,size_t len,int flags);
				...
				len
				flag:
					0：阻塞接收，是把传输层接收缓冲区的数据给拿出来，也就清空了缓冲区len的长度
					MSG_PEEK：探测接收,根据len所描述的长度去，传输层TPC接收缓冲区里"拷贝"
...
原因：1.accept函数在调用时，如果没有客户端发起连接，则会阻塞
