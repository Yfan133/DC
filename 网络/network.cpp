...
如何解决TCP粘包问题？
	...
	工业上使用的是 \r\n (0D 0A)来进行区分，
		对于应用层程序而言，1+10 这条数据
	应用层数据报头(结构体或者数据结构) + 应用层 + 数据边界
		报头用于：定义数据长度，数据种类，属性
		例如：size = 3 | 1 + 1 | 0D0A | size = 4 | 12 + 2 | 0D0A
	小结：
		1.定长数据包 + 数据(有效载荷) + 分隔符
	...
	http协议：https是加密版本的http协议
		1.超文本传输协议
		2.URL的组成：http://user:passwd@www.baidu.com:80/dir/index/html?id=1&wd=C%2B%2B
			1.http://				协议方案名
			2.user:passwd:			用户名和密码
			3.www.baidu.com			域名，最终会解析成为服务端ip
			4.80					端口
			5./dir/index/html		带层次的文件路径，表示浏览器要请求的资源路径；'/'并不是Linux操作系统的根目录，而是逻辑上的根目录，web服务器定义的根目录
			6.id = 1&wd = C%2B%2B	查询字符串，格式为key = value&key = value 形式
				由于value当中可能具有普遍标识的字符，这些字符有可能会导致歧义，所以在查询字符串的时候，需要进行URLEncode(url编码)，在服务端进行url解码
				url编码：将特殊的字符按照"16进制"传输，例如C--》2B，为了标识这个字符是经过url编码的，所以在编码完成的字符之前加上一个%，来标识后面的字符是进行过uel编码的，即C%2B%2B代表C++
