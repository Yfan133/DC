...
TCP协议：
	...
	面向字节流：TCP协议下发送方是调用send之后将数据包放到发送缓冲区当中，当满足一定条件时才进行发送，接收方数据也可以在接收缓冲区中任意长度取，这就造成了TCP粘包问题
	1.面向连接  
		问题：谈谈你所理解的三次握手？
		   从下面三个方面回答：
...
为什么会产生TCP粘包问题？以及解决？
	1.发送端调用send之后，就将数据交付给内核缓冲区了。
	2.当满足一定条件时(等待时间到了、缓冲区满了)，内核缓冲区才会进行发送，此时缓冲区中的数据是没有间隔的因此造成粘包
	3.而当数据大小超过MSS时，也会分包进行发送，对端接收时不知道哪一个数据属于哪一个分包的

	解决：
	1.不定长的数据包：首部+分隔符(\r\n)，首部包含数据的长度，尾部标识这个数据包结束
	2.定长数据包：首部+分隔符(\r\n)，首部包含数据的长度，不够长度的用空格补齐
