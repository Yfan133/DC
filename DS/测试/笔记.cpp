...
进程通信的方式：
    ...
    匿名管道特点：
        1.只适用于具有亲缘关系的进程，没有标识符。子进程拷贝父进程的task_struct结构体，其中也拷贝了文件描述符表，也就可以实现匿名管道通信
        2.在内核中，生性周期跟随进程，半双工：数据只能由一端流向另一端
    ...
    共享内存:
        1.最快的进程通信方式
        2.虽然不同进程的虚拟内存地址不同，但是映射同一磁盘空间，而虚拟地址中有一块共享区，映射同一块物理磁盘
...
互斥锁的底层有一个互斥量，本质是计数器，有0和1，如果为0则调用阻塞

死锁：
    一个执行流拿到锁资源，没有进行释放就退出了，其它执行流无法获取锁资源
