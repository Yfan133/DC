...
LZ77压缩算法：
	GZIP：使用(偏移量+长度)
	如果把偏移量设置为两个字节：最长寻找距离65535
	优点：可以在整个匹配缓冲区中寻找，找到匹配的概率非常高
	缺点：距离越远，从从局部性原理考虑：找到重复32768
	<长度:8，距离:15>

	长度为1字节(3~258)，距离为 15 比特：
		原因：距离最大是32768(32k缓冲区)，但是一般不会出现65536的长度重复32768，也就是重复了一半数据
		因此：一个替换对总大小：15比特 + 8比特 = 23比特
		                         三个字符总大小：24比特
结论：若距离当前位置32768(32k)位置以内，出现三个字节重复，则进行替换

内存缓冲区大小：64k = 65536 字节
	1.以start分两个区间：查找缓冲区和先行缓冲区
		MIN_MATCH = 3字节
		MAX_MATCH = 258字节
		MIN_LOOKHEAD = MAX_MATCH + MIN_MATCH + 1 = 261;
		WSIEZE = 32K --> 0x8000
	2.滑动窗口大小：WSIZE = 32K
		整个匹配是在一个滑动窗口中进行的
...
哈希表：64K的大小
	两个区间：prev区间：0~32k   head区间：32~64k。查找缓冲区和先行缓冲区
...
插入算法：参数：
	...
	3.匹配串的地址 match 赋值为 head[hashAddr]
	4.给 head[hashAddr] 重新赋值为 pos(新匹配串的位置)
	小结：
		1.刚进来时，head[hashAddr]记录的是上次匹配字符串的首地址
		2.则需要重新把上次匹配的首地址赋给左边新地址： prev[pos]
		3.pos & mask 原因：防止pos跑到滑动窗口的高地址，造成这里prev[pos] 跑到head空间里去
			后果：1.可能&之后的位置是其他链的节点，更甚者造成环型结构
问题：
	1.为什么不用链表？
		用数组的方式可以大大节省空间
	2.为什么要两个哈希分区？
		和缓冲区呼应，匹配完成的字符丢弃到prev区间，新匹配的字符串放到head区间
*/
/*
重命名
